...
trait Example {
  ...
  /** The next step is to define our services. For demonstration purposes I am going to use these
    * simple services' interfaces. Each one of them contains a single operation that is used by overlying services - `Layer1` uses `Layer2`, `Layer2` uses `Layer3` and so on.
    */
  trait Layer1[F[_]]:
